...
每个CPU requester的基础thread OSTD MUST 可独立配置。Web旧配置未提供每requester值时 MUST 继承全局默认Thread OSTD。
...
#### Scenario: 不同requester使用不同基础OSTD

- **WHEN** cpu0.t0配置32且cpu1.t0配置4
- **THEN** 两个requester MUST 分别以32和4作为线程级准入上限
- **AND** Core池和CBusy cap MUST 在各自基础上继续钳位有效OSTD

...
- **THEN** RSP flit携带CBusy返回RN，RN更新`(MC, PARTID)`本地表为Level 2

#### Scenario: CBusy随最后DAT flit返回更新

- **WHEN** MC完成read请求（多DAT flit）并采样CBusy Level 1
- **THEN** 仅最后一个DAT flit到达时更新RN本地表，前续flit不触发更新

#### Scenario: 无返回时保持陈旧状态

- **WHEN** CBusy档位降低但目标PARTID无新请求发往该MC
- **THEN** RN保持旧档位直至有新返回携带更低档位更新

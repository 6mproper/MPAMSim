...
- L3增加独立clock、256拍监控周期和history/current滤波权重。
- 每周期发布所有PARTID的raw抽样占用、filtered MPAM占用、物理全量占用和raw/filtered访问带宽。
- CMIN/CMAX只读取上一发布filtered sampled-owner值，不读取即时或全量隐藏状态。

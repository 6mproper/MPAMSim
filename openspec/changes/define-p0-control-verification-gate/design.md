...
- 已发布并保存的control sampled owner计数；
...
-> allocation/eviction/bypass -> raw sampled/control input/actual曲线 -> UI事件
```
...
- 未授权actual读取：L3 CMIN/CMAX使用`_control_sampled_counts`，actual occupancy只在snapshot导出；MC BMIN/BMAX使用control bandwidth状态。
- 时序：L3和MC都按monitor period发布control input；hard BMAX已有监控边界门控测试；L3 control sampled输入已有测试。

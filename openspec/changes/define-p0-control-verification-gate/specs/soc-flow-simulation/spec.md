...
- **THEN** 控制输入 MUST 来自L3发布并保存的control sampled owner
- **AND** physical actual occupancy只允许用于显示、导出和误差验证

#### Scenario: MC控制输入隔离

- **WHEN** BMIN、BMAX或QoS控制执行
- **THEN** 控制输入 MUST 来自MC发布并保存的control bandwidth和授权buffer状态
...
- **THEN** CMIN/CMAX在监控发布前 MUST 继续使用已保存的control sampled owner
...
- **THEN** 证据 MUST 连接激励、miss/fill、victim选择、allocation/eviction/bypass、raw sampled/control input/actual监控和UI事件

...
MC BMIN/BMAX使用上一边界锁存的control input。现有`MON-003`的更新顺序写成
“计算filtered后执行控制”，容易被实现成同一监控边界刚计算出的filtered立即驱动控制。

## 目标

- 将该工作收敛为P0/P1前置门槛：冻结“监控采样 -> 滤波发布 -> 控制读取 -> 动作生效”的时序语义。
...
- hard BMAX断言和释放都会增加一个本地监控边界的动作延迟；这是P0/P1前置门槛刻意冻结的时序语义，
  不是控制失败。

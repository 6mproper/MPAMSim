...
控制总览 MUST 将控制器实际读取的锁存值显示为control input。L3 occupancy MUST 把控制输入
和最新发布值标注为sampled owner或published sampled occupancy；MC bandwidth等速率量
...
- **WHEN** UI解释L3 filtered、control input、actual或raw
- **THEN** L3 occupancy MUST 说明filtered字段在当前实现中等价于发布的sampled owner快照

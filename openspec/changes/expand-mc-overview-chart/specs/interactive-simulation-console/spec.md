...
控制总览中的 MC “目标带/监控/实际”主图 SHOULD 使用全宽显示区域，并 SHOULD 比普通概览图提供更高的垂直绘图区，以便识别多个 PARTID 的走势。
...
- **WHEN** 控制总览显示 MC “目标带/监控/实际”主图
- **THEN** 该图 SHOULD 不与L3主图左右平分桌面结果区域
- **AND** SHOULD 给MC带宽曲线提供比普通概览图更高的canvas显示高度
- **AND** MUST 保持目标带、control input、published monitor、actual、actual MA、raw和控制事件的原有数据来源与图例语义

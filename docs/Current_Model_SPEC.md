...
- `P0`、`P1`、`P2`只表示阶段、验收等级或计划优先级。
  当前`P0`特指第0阶段的机制可信性验收。
- MPAM标识必须写成`PARTID 0`、`PARTID 1`、`PARTID 2`或`PARTID N`，
  不得缩写为`P0`、`P1`、`P2`。
...
- MPAM页签在resctrl开启时，对被启用CTRL_MON group映射的PARTID行显示
  “由resctrl接管”；该提示只表达配置优先级，不禁用MPAM直接模式字段；
...
说明窗口不得由鼠标悬停、移出、focus或blur自动触发；用户点击目标后打开，再次点击同一
目标、点击空白处、点击关闭按钮或按ESC关闭。
